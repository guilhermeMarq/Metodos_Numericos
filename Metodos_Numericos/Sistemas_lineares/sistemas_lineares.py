...
def norma(A: np.ndarray, *, norma: str="l-norma", relatorio: bool=False) -> float:
...
class SistemaLinearSolver:
    ...
    def jacobi(self, *, relatorio: bool=False, salvar: bool=False) -> np.ndarray[float]:
        ...
        while(erro >= self.e):
            ...
            erro = norma(xk - x, norma="l-norma")
            x = xk.copy()
            n_interaçoes += 1

        if relatorio:
            self.relatorio("Jacobi", x, n_interaçoes, erro)

        if salvar:
            self.salvar_em_excel(x, nome_planilha="Jacobi")
    ...
    def seidel(self, *, relatorio: bool=False, salvar: bool=False) -> np.ndarray[float]:
        ...
        while(erro_relativo >= self.e):
            ...
            x = xk.copy()
            erro_relativo = norma(x - xt, norma="l-norma")
            n_interaçoes += 1
    ...
    def relaxamento(self, *, relatorio: bool=False, salvar: bool=False) -> np.ndarray[float]:
        ...
        while(erro_absoluto >= self.e):
            ...
            xk[n_max] += delta_x
            erro_absoluto = norma(xk - x, norma="l-norma")
